...
def grid_search():
    ...
    for trial in range(10):
        # random model params
        num_epochs = 150#random.randint(150, 150)
    ...
    #=====Testing=====
    # 1) Multi-class, Clean and Noisy
    print("=====Multi-class - Clean=====")
    test_loader = DataLoader(AugmentedMFCCDataset(testing_paths, test_labels, label_map, training=False,classes_num=classes_num,is_multilabel=False,audio_augment=False,audio_params=test_audio_augmentation_params), batch_size=1, shuffle=False)
    evaluate_model_on_test(test_loader, '../' + best_model_dir, device,k=2, criteria="exact_match", is_multilabel=is_multilabel)
    print("=====Multi-class - Noisy=====")
    test_loader = DataLoader(AugmentedMFCCDataset(testing_paths, test_labels, label_map, training=False,classes_num=classes_num,is_multilabel=False,audio_augment=True,audio_params=test_audio_augmentation_params), batch_size=1, shuffle=False)
    evaluate_model_on_test(test_loader, '../' + best_model_dir, device,k=2, criteria="exact_match", is_multilabel=is_multilabel)
    # 2)  Multi-label, Clean and Noisy
    print("=====Multi-label - Clean=====")
    test_loader = DataLoader(AugmentedMFCCDataset(testing_paths, test_labels, label_map, training=False,classes_num=classes_num,is_multilabel=True,audio_augment=False,audio_params=test_audio_augmentation_params), batch_size=1, shuffle=False)
    evaluate_model_on_test(test_loader, '../' + best_model_dir, device,k=2, criteria="exact_match", is_multilabel=is_multilabel)
    print("=====Multi-label - Noisy=====")
    test_loader = DataLoader(AugmentedMFCCDataset(testing_paths, test_labels, label_map, training=False,classes_num=classes_num,is_multilabel=True,audio_augment=True,audio_params=test_audio_augmentation_params), batch_size=1, shuffle=False)
    evaluate_model_on_test(test_loader, '../' + best_model_dir, device,k=2, criteria="exact_match", is_multilabel=is_multilabel)
